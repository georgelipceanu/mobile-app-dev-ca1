...
class AuthPresenter(private val view: AuthView) {
    ...
    fun doSignUp(email: String, password: String) {
        val passRegEx = Regex("^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[^A-Za-z0-9]).{8,}$") // ref to chatgpt chat: https://chatgpt.com/share/6942f5b3-1b98-8013-8cec-ac59c2c662ca
        // if (password.length < 8) { // for demo/debug purposes only
